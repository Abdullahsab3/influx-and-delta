...
mod params;
mod stream;
pub mod influxdelta_ext_planner;
...
use crate::datafusion_utils::ThenWithOpt;
    ...
    fn compute_properties(input: &Arc<dyn ExecutionPlan>) -> PlanProperties {
        ...
        let eq_properties = match input.properties().output_ordering() {
            ...
            Some(output_ordering) => {
                EquivalenceProperties::new_with_orderings(schema, &[datafusion::physical_expr::LexOrdering { inner: output_ordering.to_vec() }])
            }
        };

        let output_partitioning = Partitioning::UnknownPartitioning(1);

        PlanProperties::new(eq_properties, output_partitioning, datafusion::physical_plan::execution_plan::EmissionType::Both, datafusion::physical_plan::execution_plan::Boundedness::Bounded)
    }
}
